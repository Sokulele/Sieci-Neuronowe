...
# Batch Update Perceptron Algorithm (BUPA) z logowaniem wag i wizualizacją granicy decyzyjnej
class BUPAPerceptronVerbose:
    def __init__(self):
    ...
    def train(self, X_train: list[list[float]], y_expected: list[float], epochs: int, learning_rate: float) -> None:
        ...
        for epoch in range(epochs):
            ...
            for i, x in enumerate(X_train):
                ...
                print(f"  Iteracja {i+1}: x={x.tolist()}, y (wartość oczekiwana) = {y_expected[i]}, f(v)={int(y_pred)}, error(d-y)={error}")
            ...
            if errors == 0:
                ...
                self.plot_decision_boundary(epoch)
                plt.savefig('bupa_final_decision_boundary.png')
                break
...
# Trening perceptronu metodą BUPA
perceptron_bupa = BUPAPerceptronVerbose()
perceptron_bupa.train(x_train, y_train, epochs=10, learning_rate=1.0)
